			...
			<?php 
				...
				for($cb=1;$cb<=$boxDisplay;$cb++){
					if(($cb >= $currentMonthFirstDay || $currentMonthFirstDay == 1) && $cb <= ($totalDaysOfMonthDisplay)){
						...
						global $db;
                        $result = $db->query("SELECT title FROM classdates WHERE date = '".$currentDate."' AND status = 1");
						...
						if(strtotime($currentDate) == strtotime(date("Y-m-d"))){
							echo '
								<div class="calendar__day today" onclick="getEvents(\''.$currentDate.'\');">
									...
									<span class="calendar__task calendar__task--today">'.$eventNum.' Classes</span>
								</div>
							';
						}elseif($eventNum > 0){
							echo '
								<div class="calendar__day event" onclick="getEvents(\''.$currentDate.'\');">
									<span class="calendar__date">'.$dayCount.'</span>
									<span class="calendar__task">'.$eventNum.' Classes</span>
								</div>
							';
						}else{
							echo '
...
function getEvents($date = ''){
	...
	global $db;
	$result = $db->query("SELECT title FROM classdates WHERE date = '".$date."' AND status = 1");
